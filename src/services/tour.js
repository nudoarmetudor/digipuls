...
const CHAPTERS = [
  ...
  // ------------------------------------------------------------ assessment
  {
    ...
    steps: [
      ...
      S('start', '[data-tour="start-cycle"]', '/school',
        ...
        'school.manage'),
      S('delegation', '[data-tour="delegation-link"]', null,
        L('Share the nineteen parameters out',
          'Repartizați cei nouăsprezece parametri',
          'Распределите девятнадцать параметров'),
        L('Nineteen parameters is more than one person can evidence properly, and your team is five or six. Agree who gathers what and write it here, so nothing is left for somebody else to notice. It is a plan of work, not a restriction — anyone can still record a level for any parameter on their own side.',
          'Nouăsprezece parametri sunt mai mult decât poate documenta temeinic o singură persoană, iar echipa dumneavoastră are cinci-șase membri. Conveniți cine ce adună și scrieți aici, ca nimic să nu rămână în seama altcuiva. Este un plan de lucru, nu o restricție — oricine poate în continuare înregistra un nivel pentru orice parametru, pe partea sa.',
          'Девятнадцать параметров — больше, чем один человек может как следует обосновать, а в вашей команде пять-шесть человек. Договоритесь, кто что собирает, и запишите здесь, чтобы ничто не осталось на чужое усмотрение. Это план работы, а не ограничение: любой по-прежнему может записать уровень по любому параметру на своей стороне.'),
        'school.manage'),
      ...
      S('evidence', '[data-tour="evidence"]', null,
        ...
        L('A document, a link, a named practice — whatever a sceptical reader would need in order to believe the rating. Describe it, and attach the file itself or a link to where it is kept. Levels 0 and 1 need none. A rating of 2 or more without evidence turns its step red and will stop the cycle being confirmed.',
          'Un document, un link, o practică denumită — orice i-ar trebui unui cititor sceptic ca să creadă evaluarea. Descrieți-o și atașați fișierul sau un link către locul unde se păstrează. Nivelurile 0 și 1 nu au nevoie de dovezi. O evaluare de 2 sau mai mult fără dovezi face pasul roșu și va împiedica confirmarea ciclului.',
          'Документ, ссылка, названная практика — всё, что нужно скептику, чтобы поверить оценке. Опишите доказательство и прикрепите сам файл или ссылку на место хранения. Уровням 0 и 1 доказательства не нужны. Оценка 2 и выше без доказательства делает шаг красным и не даст подтвердить цикл.')),
  ...
  // --------------------------------------------------------- reconciliation
  {
    ...
    steps: [
      S('open', '[data-tour="reconcile-link"]', null,
        ...
        L('The two readings are put side by side. Until you have answered a parameter yourself, the other side’s level for it stays hidden — seeing their answer first would turn an assessment into a negotiation, so the platform holds the two apart rather than asking you to.',
          'Cele două evaluări sunt așezate una lângă alta. Până când evaluați dumneavoastră un parametru, nivelul celeilalte părți pentru el rămâne ascuns — a le vedea răspunsul mai întâi ar transforma evaluarea într-o negociere, așa că platforma le ține separate în locul dumneavoastră.',
          'Две оценки помещаются рядом. Пока вы сами не оценили параметр, уровень другой стороны по нему скрыт: увидеть их ответ первым значило бы превратить оценивание в переговоры, поэтому платформа держит стороны раздельно за вас.')),
  ...
  // --------------------------------------------------------------- renewal
  {
    key: 'renew',
    need: 'view.school',
    title: L('The end of the plan, and the next cycle', 'Finalul planului și ciclul următor', 'Конец плана и следующий цикл'),
    intro: L(
      'Two years on: report on the second year, see what moved, and assess again.',
      'După doi ani: raportați asupra celui de-al doilea an, vedeți ce s-a schimbat și evaluați din nou.',
      'Через два года: отчитайтесь за второй год, посмотрите, что изменилось, и оцените снова.'),
    steps: [
      S('final', '[data-tour="report-year-one"]', null,
        L('The final report keeps year one beside year two',
          'Raportul final păstrează primul an alături de al doilea',
          'Итоговый отчёт показывает первый год рядом со вторым'),
        L('Each measure has a result for each year. What you record now goes into the second-year column; the first-year figure stays where it was, so the report shows the movement between the two.',
          'Fiecare măsură are un rezultat pentru fiecare an. Ce înregistrați acum intră în coloana celui de-al doilea an; cifra primului an rămâne unde era, astfel încât raportul arată evoluția dintre cei doi ani.',
          'У каждого показателя есть результат за каждый год. То, что вы записываете сейчас, попадает в столбец второго года; цифра первого года остаётся на месте, и отчёт показывает изменение между ними.')),
      S('history', '[data-tour="history-table"]', '/school/history',
        L('What moved, and whether the plan got there',
          'Ce s-a schimbat și dacă planul și-a atins ținta',
          'Что изменилось и достиг ли план цели'),
        L('Once a second cycle is confirmed, each parameter shows whether it grew, held or fell — and, where the last plan set out to advance it, whether the level it aimed for was reached.',
          'După confirmarea celui de-al doilea ciclu, fiecare parametru arată dacă a crescut, s-a menținut sau a scăzut — și, acolo unde planul anterior își propunea avansarea, dacă nivelul vizat a fost atins.',
          'После подтверждения второго цикла у каждого параметра видно, вырос ли он, удержался или снизился, а там, где прошлый план ставил цель продвинуться, — достигнут ли намеченный уровень.')),
      S('start', '[data-tour="start-cycle"]', '/school',
        L('Starting the next cycle',
          'Începerea ciclului următor',
          'Начало следующего цикла'),
        L('Both sides assess from a blank page again, so the new reading is a finding rather than a copy. Last cycle\'s agreed level is shown beside every parameter, and the equipment data — write-offs included — carries over for you to update.',
          'Ambele părți evaluează din nou de la zero, astfel încât noua evaluare este o constatare, nu o copie. Nivelul convenit în ciclul anterior apare lângă fiecare parametru, iar datele despre echipament — inclusiv casările — se preiau, ca să le actualizați.',
          'Обе стороны снова оценивают с чистого листа, чтобы новая оценка была выводом, а не копией. Согласованный в прошлом цикле уровень показан рядом с каждым параметром, а данные об оборудовании — включая списание — переносятся, чтобы вы их обновили.'),
        'school.manage'),
      S('split', '[data-tour="delegation-link"]', null,
        L('Last cycle\'s split of the work carries over',
          'Împărțirea sarcinilor din ciclul anterior se preia',
          'Распределение работы из прошлого цикла переносится'),
        L('Everyone still on the team keeps the parameters they took last time. People who have left are dropped. Check it before the team starts.',
          'Fiecare membru rămas în echipă păstrează parametrii preluați data trecută. Cei care au plecat sunt eliminați. Verificați înainte ca echipa să înceapă.',
          'Все, кто остался в команде, сохраняют параметры, взятые в прошлый раз. Ушедшие исключаются. Проверьте распределение до начала работы.'),
        'school.manage'),
      S('close', '[data-tour="close-cycle"]', null,
        L('Closing the cycle',
          'Închiderea ciclului',
          'Закрытие цикла'),
        L('When the final report is out, record that the cycle is finished. It is a statement, not a lock: the plan and the reports stay editable, the card lists anything not yet published, and the cycle can be reopened.',
          'Când raportul final a fost publicat, consemnați că ciclul s-a încheiat. Este o declarație, nu o blocare: planul și rapoartele rămân editabile, cardul arată ce nu a fost încă publicat, iar ciclul poate fi redeschis.',
          'Когда итоговый отчёт опубликован, отметьте, что цикл завершён. Это заявление, а не блокировка: план и отчёты остаются редактируемыми, карточка показывает, что ещё не опубликовано, а цикл можно открыть снова.'),
        'school.manage'),
      S('previous', '[data-tour="plan-previous"]', null,
        L('The new plan remembers the old one',
          'Noul plan își amintește de cel vechi',
          'Новый план помнит старый'),
        L('When you open the plan for the new cycle, each parameter shows what the last plan aimed for and whether it was reached — the place to decide whether to aim again, aim higher, or hold.',
          'Când deschideți planul noului ciclu, fiecare parametru arată ce viza planul anterior și dacă ținta a fost atinsă — locul în care decideți dacă vizați din nou, mai sus sau mențineți.',
          'Когда вы откроете план нового цикла, у каждого параметра будет видно, на что был нацелен прошлый план и достигнута ли цель, — здесь решается, ставить ли цель снова, выше или удерживать уровень.')),
    ],
  },
