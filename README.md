...
[PROCESS.md](PROCESS.md) describes everything the platform does, from provisioning an account to publishing the final report on a two-year plan — the instrument, the positions, and each process step by step, including the places where the software deliberately refuses and why. It is the source text for the user manual.

The same material is in the product: signed-in school accounts get a guided tutorial (the **Guide** button in the masthead) that highlights the actual control on the actual page, chapter by chapter. See `src/services/tour.js`.
...
## New in this pass: three languages, accessibility, and a brand identity
...
- 19 sectors, one per indicator, grouped contiguously by domain (A/B/C/D coloured across the two DigiProf brand ramps). Ring distance from centre = the indicator’s current level (0 = centre hole, 5 = outer edge). Hover any sector for the exact level; the same levels are always listed as text in the table beside it, so the picture is a summary rather than the only copy of the data. Sector colours are theme-aware — left at their light-theme values the wheel would be four dark smudges on a dark background.
...
**Three languages — English, Romanian and Russian** — covering the instrument itself, not just the chrome:
- **The entire 19-indicator, 6-level instrument exists in all three.** `src/data/indicators.js`, `indicators.ro.js` and `indicators.ru.js` mirror each other exactly — same codes, same order, same six levels, same quantitative benchmarks — differing only in the displayed text. `src/data/indicatorsI18n.js` is the dispatcher every route calls for display.
- **Russian is there because a substantial share of Moldovan schools work in it**, particularly in the north and south. A self-assessment a school's team can't read in its working language is a translation exercise, not a self-assessment.
- **Every user-facing string goes through `t()`.** The previous pass used inline `lang === 'ro' ? ... : ...` conditionals in the views, which worked for two languages and silently served English to everyone else. Those are gone; `src/i18n/index.js` is now the single dictionary, with `{placeholder}` interpolation for messages that carry a number or a name.
- **A missing translation fails the build.** `tests/i18n.test.js` compares the three dictionaries key by key, rejects a "translation" that is still the English string (with an explicit allow-list for genuine cognates like *Contrast* and *Standard*), checks that placeholders survived translation, and verifies the three indicator datasets are structurally identical. `tests/views.test.js` renders every page template in all three languages and fails if an untranslated key leaks into the HTML.
- **Language selection**: session-persisted for signed-in users, cookie-persisted for everyone else, and negotiated from `Accept-Language` for a first-time visitor who hasn't chosen (`mo` is accepted as the deprecated code for Romanian). Switching is available on every page, including the login screen and the public pages.

**Display and accessibility preferences** — this is a national platform that school staff are required to use, so there is no fallback audience:
- **Colour scheme** (light / dark / follow the system), **high contrast** (AAA pairings built from the same two brand ramps, links always underlined), **text size** (100/115/130/150%), **reduced motion**, and **always-underline-links** — all from a menu on every page.
- Preferences are stored **per browser** — a cookie the server reads plus `localStorage` — never against the user account. So they apply on the first render with no flash of the wrong theme, they work before login and on the public pages, and they work with JavaScript switched off (the panel is a plain `<details>` + `<form>` posting to `/preferences`; `public/js/a11y.js` only upgrades it to apply instantly). A display setting shouldn't become another field of personal data about a named school employee.
- Text sizing works by scaling the root font size, and every length in `public/css/style.css` is in `rem` — so the interface reflows at 150% rather than overflowing. Verified at 390px wide with the largest text size: no horizontal page scroll.
- Wide tables scroll inside their own region rather than the page, and that region becomes keyboard-focusable **only when it actually overflows**, so no pointless tab stops appear on narrow tables.
- Semantic landmarks, a skip link, labelled controls throughout, `aria-current` on the current page and step, live regions for status changes, 44px minimum targets, and visible `:focus-visible` rings. The step-navigator's coloured dot is decorative — its state is repeated as text, so colour never carries meaning alone.

**Brand identity** — see [BRAND.md](BRAND.md). The mark is four arcs (the four MDSF domains, echoing the maturity wheel) broken by a pulse line running through and out both sides; the ring is left open because digital maturity is never finished. Logo files are in `public/img/`. The stylesheet is now token-driven: components reference semantic tokens (`--surface`, `--text`, `--primary`), never raw shades, which is what makes the four viewer preferences possible at all.
...
| Languages | EN / RO / RU, including the full instrument; enforced by tests. No Gagauz yet |
| Display & accessibility preferences | Implemented — light/dark/system, high contrast, four text sizes, reduced motion, underline links; cookie-backed, works without JavaScript |
| Automated tests | Partial — `npm test` covers Order 675 rules, draft/confirmed selection, step-status colours, input validation, translation parity, preference handling, and renders every view in all three languages; no route-level/integration tests yet |
...
- **Gagauz.** EN/RO/RU are complete across every view and the full instrument, and the test suite keeps them in step. Gagauz was flagged in the design docs as a possible future need; adding it is additive work against `src/i18n/index.js` plus one more `indicators.*.js` file, not a redesign.
- **A formal accessibility audit.** The interface was built to WCAG 2.1 AA intent and checked by hand — keyboard path, focus visibility, landmark structure, contrast pairings, reflow at 150% text and 390px width — but it has not been through an audit with real assistive-technology users, which is the only test that actually counts. See ROADMAP.md.
